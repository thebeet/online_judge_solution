...
// Andrew Algorithm 
const convexHull2D = (p: Vector2[]): Vector2[] => {
    const n = p.length;
    if (n <= 2) {
        return p;
    }
    ...
    return stack.slice(1, stack.length).map(index => points[index]);
};

function outerTrees(trees: number[][]): number[][] {
    return convexHull2D(trees.map(t => new Vector2(t[0], t[1]))).map(v => ([v.x, v.y]));
};