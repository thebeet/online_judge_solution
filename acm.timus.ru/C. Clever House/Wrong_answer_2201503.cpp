...
class looong
{
	...
	friend long long operator % (const looong & d1, const long long m);
...
long long operator % (const looong & d1, const long long m)
{
	 long long rest = 0;
	for (int i = d1.length; i >= 1; --i)
	{
		rest = (rest * d1.step + (long long)d1[i]) % m;
	}
	return rest;
}
