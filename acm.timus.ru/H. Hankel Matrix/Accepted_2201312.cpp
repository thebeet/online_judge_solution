...
typedef looong T_INT;

T_INT matrix[2][128][128];

T_INT rr(const int p, const int pos, const T_INT m)
{
	T_INT ans(1);
	int pos2 = (pos + 1) / 2;
	for (int i = 0; i <= pos2; ++i)
	{
		...
		for (int j = 0; j < i; ++j)
		{
			matrix[p][i][pos - i] -= matrix[p][j][pos - i] * matrix[p][j][i];
		}
		ans += matrix[p][i][pos2] * matrix[p][i][pos2];
	}
	return ans;
}
...
int main()
{
	...
	for (int i = 1; i <= n; i += 2)
	{
		ans[i] = rr(0, i, ans[i - 1]);
		rr(0, i + 1, ans[i]);
		ans[i + 1] = rr(1, i, ans[i]);
		rr(1, i + 1, ans[i + 1]);
	}
	cout << 1 << endl;
	for (int i = 0; i < n - 1; ++i)
