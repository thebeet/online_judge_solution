...
double calc_angle(t_Point &po, t_Point &p1, t_Point &p2)
{
	double a1 = (p1 - po).angle();
	double a2 = (p2 - po).angle();
	double a = a1 - a2;
	while (a < 0.0) a += 360.0;
...
int main()
{
	...
	inp.vertexs[0] = inp.vertexs[n];
	inp.vertexs[n + 1] = inp.vertexs[1];
	inp.size = n;
	double minangle = PI;
	for (i = 1; i <= n; ++i)
	{
		minangle = min(minangle, calc_angle(inp.vertexs[i], inp.vertexs[i - 1], inp.vertexs[i + 1]));
		for (j = i + 2; j <= n; ++j)
		{
			//if ((i == 1) && (j == n))
			double a1 = calc_angle(inp.vertexs[i], inp.vertexs[j], inp.vertexs[i + 1]);
			double a2 = calc_angle(inp.vertexs[j], inp.vertexs[i], inp.vertexs[j + 1]);
			minangle = min(minangle, a1 + a2);
			a1 = calc_angle(inp.vertexs[i], inp.vertexs[j], inp.vertexs[i - 1]);
			a2 = calc_angle(inp.vertexs[j], inp.vertexs[i], inp.vertexs[j - 1]);
			minangle = min(minangle, a1 + a2);
		}
	}
    printf("%.13lf\n", minangle);
