...
bool cmp(t_Point p1, t_Point p2)
{
...
int main()
{
	while (gets(buf) != NULL)
	{
		...
		while (sscanf(buf + dx, "%s", buf2) != EOF)
		{
			++n;
			sscanf(buf2, "(%lf,%lf)", &inp.points[n].x, &inp.points[n].y);
			dx += strlen(buf2);
			for (; (buf[dx] != '(') && (buf[dx] != '\0'); ++dx)
			{
			}
			if (buf[dx] == '\0')
			{
		...
		{
			printf("(%lf,%lf)%c", out.points[i].x, out.points[i].y, (i == out.size) ? '\n' : ' ');
		}
	}
	return 0;
}
