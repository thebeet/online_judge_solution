...
int main()
{
	...
	while (gets(buf) != NULL)
	{
		...
		for ( ; ; )
		{
			...
			++n;
			inp.vertexs[n].start = dx; 
			if (sscanf(buf + dx, "(%lf,%lf)", &inp.vertexs[n].x, &inp.vertexs[n].y) != 2)
			{
				--n;
				break;
			}
		...
		for (i = 1; i <= stack[0]; ++i)
		{
			for (int j = inp.vertexs[stack[i]].start; j <= inp.vertexs[stack[i]].end; ++j)
			{
				printf("%c", buf[j]);
			}
			printf("%c", (i == stack[0]) ? '\n' : ' ');
