...
public:
	SuperSimpleNetWork(istream &in)
	{
		...
		for (int i = 0; i < m_n; ++i)
		{
			AddPath(CowNodeID(i), CowNodeID_1(i));
			int fn, dn;
			in >> fn >> dn;
			for (int j = 0; j < fn; ++j)
			{
			...
			for (int j = 0; j < dn; ++j)
			{
				...
				in >> did;
				--did;
				AddPath(CowNodeID_1(i), DrinkNodeID(did), 2);
			}
		}
	}
...
private:
	...
	int CowNodeID_1(const int c) const
	{
		return c + m_n + 2;
	}
