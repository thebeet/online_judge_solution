...
public:
	...
	t_Point(const double new_x, const double new_y, const double new_z)
	{
		x = new_x;
		y = new_y;
		z = new_z;
...
int main()
{
	...
	{ 
		...
		{
			cin >> segments[i];
			points[i * 2 - 1] = segments[i].p1;
			points[i * 2] = segments[i].p2;
			t_Line newline(segments[i].p1, segments[i].p2);
			lines[i] = newline;
		}
		for (i = 1; i <= n + n; ++i)
		{
			for (j = i + 1; j <= n + n; ++j)
			{
				...
				{
					t_Line sline(points[i], points[j]);
					for (k = 1; k <= n; ++k)
					{
						t_Point cp;
						if (linecross(sline, lines[k], cp) == 0)
						{
							if (dblcmp(sline.a + sline.b + sline.c - lines[k].a - lines[k].b - lines[k].c) == 0)
							{
								continue;
							}
							break;
						}
						if (between(cp, segments[k]) == 1)
						{
							break;
						}
					}
