...
public:
	t_Segment(const t_Point &new_p1 = t_Point(0, 0), const t_Point &new_p2 = t_Point(0, 0))
	{
		p1 = new_p1;
		p2 = new_p2;
	}
...
typedef pair<double, double> shadow;

bool operator < (const shadow &s1, const shadow &s2)
{
...
int main()
{
	...
	for (int ti = 1; ti <= tn; ++ti)
	{
		...
		for (i = 0; i < n; ++i)
		{
			t_Segment seg;
			cin >> seg;
			shadows[i].first = o.x + (seg.p1.x - o.x) / (seg.p1.y - o.y) * o.y;
			shadows[i].second = o.x + (seg.p2.x - o.x) / (seg.p2.y - o.y) * o.y;
			if (shadows[i].first > shadows[i].second)
			{
				swap(shadows[i].first, shadows[i].second);
			}
		}
		sort(shadows, shadows + n);
		double lo = -BIG;
		int sum(1);
		for (i = 0; i < n; ++i)
		{
			if (shadows[i].first > lo)
			{
				++sum;
			}
			lo = max(lo, shadows[i].second);
		}
		cout << sum << endl;
	}
	return 0;
