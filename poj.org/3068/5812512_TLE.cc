...

{
	...
	friend int SPFACost(const GraphWeightCost &g, const int st, const int ed, int pre[])
	...
	friend pair<int, int> MinCost(GraphWeightCost &g, const int st, const int ed)

	{
		...
		while ((totalcost = SPFACost(g, st, ed, pre)) < INF)

		{
...

			{
				...
				for (int pp = g.path_head[pre[i]]; pp != -1; pp = g.path_next[pp])

				{

					if ((g.path_to[pp] == i) && (g.path_weight[pp] > 0))

					{
						...
						if (g.path_cost[pp] < g.path_cost[minpp])
				...
				neck = min(neck, g.path_weight[minpp]);
