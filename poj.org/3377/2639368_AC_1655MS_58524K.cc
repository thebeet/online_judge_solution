...
long long path[2097152];
long long sea[1048576];
...
long long dijkstra(int pstart, int pend)
{
	...
	for ( ; ;)
	{
		...
		{
			new_data.weight = st_data.weight + sea[st_data.data >> 1];
			heap.insert(new_data);
		}
		new_data.data = st_data.data - 2;
		if ((new_data.data >= 0) && (gone[new_data.data] == false))
		{
			new_data.weight = st_data.weight + path[new_data.data];
			heap.insert(new_data);
		}
		new_data.data = st_data.data + 2;
		if ((new_data.data <= n2) && (gone[new_data.data] == false))
		{
			new_data.weight = st_data.weight + path[st_data.data];
			heap.insert(new_data);
		}
	}
