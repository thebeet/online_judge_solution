	...
	*const_cast <bool *>(&(d2.op)) = false;
	...
	for (int i = d3.length; i >= 1; --i)
	{
		...
		while (lo + 1 < hi)
		{
			int mid = (lo + hi) >> 1;
			d3[i] = mid;
			//int compret = compare(partmul(d3, d2, i, 1), d1);
			int compret = compare(d3 * d2, d1);
			switch (compret)
			{
			case 0:
				d3.declength();
...
looong sqrt(const looong &d1)
{
	...
	for (i = ans.length; i >= 1; --i)
	{
		...
		while (lo + 1 < hi)
		{
			...
			int compret = compare(ans * ans, d1);
			switch (compret)
			{
			case 0:
				ans.declength();
...
int main()
{
	...
	while ((cin >> n >> m), n > 0)
	{
		...
		looong ans(1);
		for (looong i(1); i <= n; ++i)
		{
			ans *= i;
		}
		for (looong i(1); i <= n - m; ++i)
		{
			ans /= i;
		}
		for (looong i(1); i <= m; ++i)
		{
			ans /= i;
		}
