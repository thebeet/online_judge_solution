...
int main()
{
	...
	for (ti = 1; ti <= tn; ++ti)
	{
		...
		sort(plants + 1, plants + 1 + n);
		memset(usage, true, sizeof(usage));
		int pr = 1;
		cout << n << " " << plants[1];
		for (k = 2; k <= n; ++k)
		{
			...
			for (i = 2; i <= n; ++i)
			{
				if (usage[i])
				{
					...
					{
						int ret = dblcmp(cross(plants[i], plants[minp], plants[pr]));
						if (ret > 0)
						{
							minp = i;
						}
						...
						{
							if (dist(plants[minp], plants[pr]) > dist(plants[i], plants[pr]))
							{
								minp = i;
							}
						}
