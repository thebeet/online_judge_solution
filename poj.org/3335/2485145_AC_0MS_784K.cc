...
int main()
{
	...
	for (ti = 1; ti <= tn; ++ti)
	{
		...
		for (i = 1; i <= n; ++i)
		{
			...
			segments[i - 1].p2 = points[i];
		}
		segments[n].p2 = points[1];
		for (i = 1; i <= n; ++i)
		{
		...
		for (i = 1; i <= n; ++i)
		{
			for (j = i + 1; j <= n; ++j)
			{
				int ret;
				t_Point cp;
				ret = linecross(lines[i], lines[j], cp);
				if (ret == 1)
				{
					points[++k] = cp;
				}
		...
		for (i = 1; i <= k; ++i)
		{
			for (j = 1; j <= n; ++j)
			{
				if (dblcmp(cross(segments[j].p1, segments[j].p2, points[i])) == 1)
				{
					break;
				}
			}
