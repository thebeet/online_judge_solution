/*
 * PKU2305::Basic_remains.cpp
 *
 *  Created on: Sep 13, 2008 7:25:29 PM
 *      Author: TheBeet
...
#endif /* UNSIGNED_ */
	bool flag(true);
	for (int w = defaultw - 1; w >= 0; --w)
	{
		int ret = ((d1[d1.length] & (1 << w)) ? 1 : 0);
		if ((ret == 0) && flag)
		{
			continue;
		}
...
int operator % (const looong & d1, const int m)
{
	...
	{
		rest = (rest * (long long)d1.step + (long long)d1[i]) % (long long)m;
	}
	return (int)rest;
}
...
{
	int b;
	while ((cin >> defaultbase), (defaultbase > 0))
	{
		looong a;
		cin >> a;
		string m;
