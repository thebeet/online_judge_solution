...
int main()
{
	...
	while (scanf("%d%d%d", &n, &d, &m), n > 0)
	{
		...
		for (it = 1; it <= 10; ++it)
		{
			for (ix = 0; ix <= n + d + d; ++ix)
			{
				for (iy = 0; iy <= n + d + d; ++iy)
				{
					for (dx = 0; dx <= d; ++dx)
					{
						for (dy = 0; dy <= d; ++dy)
						{
							if ((dx * dx) + (dy * dy) <= dd)
							{
								if ((dx == dy) && (dx == 0))
								{
									...
									dp[it][ix][iy] = max(dp[it][ix][iy], dp[it - 1][ix][iy] + sum);
								}
								else
								{
									for (op1 = -1; op1 <= 1; op1 += 2)
									{
										for (op2 = -1; op2 <= 1; op2 += 2)
										{
											int px = ix + op1 * dx;
											int py = iy + op2 * dy;
											if ((px >= 0) && (py >= 0) && (px <= n + d + d) && (py <= n + d + d))
											{
												int sum = 0;
												for (g = 0; g <= gcdans[dx][dy]; ++g)
												{
													if (mole[it][ix + dx * g / gcdans[dx][dy]][iy + dy * g / gcdans[dx][dy]])
													{
														++sum;
													}
												}
												dp[it][px][py] = max(dp[it][px][py], dp[it - 1][ix][iy] + sum);
											}
										}
									}
								}
