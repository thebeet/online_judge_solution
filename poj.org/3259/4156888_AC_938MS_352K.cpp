...
int main()
{
	...
	for (int ti = 0; ti < tn; ++ti)
	{
		int n, m, w;
		cin >> n >> m >> w;
		vector<Path> paths(m * 2 + w);
		for (int i = 0; i < m; ++i)
		{
			cin >> paths[i << 1].f >> paths[i << 1].t >> paths[i << 1].len;
		...
		for (int i = m; i < w; ++i)
		{
			cin >> paths[i].f >> paths[i].t >> paths[i].len;
