...
public:
	...
	{
		int i, j;
		int t_fill = 0x1FF8; //111111111111111100010000
		for (i = 0; i < size; ++i)
		{
			for (j = 0; j < size; ++j)
			{
			...
			rect[i] = row[i] = clm[i] = 0x1FF; //11111111111111111
		}
		complete = 0;
	}
	...
	for (;;)
	{
		...
		for (i = 0; i < size; ++i)
		{
			for (j = 0; j < size; ++j)
			{
				if (inp[i * size + j] != '.')
				{
					sudoku.add(i, j, inp[i * size + j] - '1');/* == false)
					{
						printf("error\n");
					}*/
				}
