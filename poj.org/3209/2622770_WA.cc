...
void prime_init(long long primsize = 46340)
{
	long long i, j;
...
T_INT calc_f1(T_INT m)
{
	...
	int ans = phi;
	Factors<T_INT> f = Factorizate(phi);
...
int main()
{
	...
	for (ti = 1; ti <= tn; ++ti)
	{
		int i;
		cin >> n;
		Factors<long long> f = Factorizate(n);
		for (i = 1; i <= f.n; ++i)
		{
			if ((f.p[i] <= 2097151) && ((f.p[i] & 3) == 3) && (f.l[i] & 1))
			{
				break;
			}
		}
		if (i == f.n)
		{
			if (f.p[i] > 2097151)
			{
				if ((primetest(f.p[i])) && ((f.p[i] & 3)== 3))
				{
					cout << "NO" << endl;
				}
				else
				{
					long long df = Pollard(f.p[i]);
					if (df * df == f.p[i])
					{
						cout << "YES" << endl;
					}
